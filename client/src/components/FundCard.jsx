...
const FundCard = ({owner,title,description,target,deadline,amountCollected,image,handleClick}) => {
  ...
  return (
    <div className='sm:w-[288px] w-full rounded-[15px] bg-[#1c1c24] cursor-pointer ' onClick={handleClick}>
        <img src={image} alt="CampaignImage" className='w-full h-[158px] object-cover rounded-[15px] '/>

        <div className='flex flex-col p-4 '>
            <div className='flex flex-row items-center mb-[18px] '>
                <img src={tagType} alt="tag" className='w-[17px] h-[17px] object-contain ' />
                <p className='ml-[12px] mt-[2px] font-medium text-[12px] text-[#808191] '>Category</p>
            </div>
            <div className='block'>
                <h3 className='font-semibold test-[16px] text-white text-left truncate '>{title}</h3>
            ...
            <div className='flex items-center mt-[20px] gap-[12px] '>
                ...
                <p className='flex-1 font-normal text-[#808191] truncate '>by <span className='text-[#b2b3bd]'>{owner}</span></p>
            </div>
        </div>
